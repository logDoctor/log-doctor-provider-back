...
### 0. 시스템 전체 아키텍처 (High-Level Overview)

```mermaid
graph TD
    subgraph "Provider Environment (공급자 클라우드)"
        PB["<b>Provider Backend</b><br/>(FastAPI / Clean Arch)"]
        DB[("<b>Cosmos DB</b><br/>(Metadata)")]
        Repo["<b>Package Repo</b><br/>(Agent Zip Files)"]
        PB -- "Managed Identity" --- DB
        PB -- "Local FS" --- Repo
    end

    subgraph "Operator / CI-CD"
        Op["운영자 & GitHub Actions"]
        Op -- "(1) 패키지 업로드" --> PB
    end

    subgraph "Customer Azure Environment"
...
├── models.py        # 도메인 엔티티 (비즈니스 로직 포함)
├── router.py        # 엔드포인트 정의
├── schemas.py       # API 데이터 모델 (Pydantic)
├── dependencies.py  # 의존성 주입 (Provider)
├── repository.py    # 데이터 접근 추상화 (ABC Interface + AOP Implementation)
└── usecases/        # 단일 책임 비즈니스 로직 (Use Case Pattern)
    ├── use_case_1.py
    └── use_case_2.py
```
...
### 2. Repository Pattern & AOP Mapping (DRY)

비즈니스 로직은 실제 DB 엔진을 몰라야 하며, 로우(Raw)한 데이터(`dict`)와 대화하지 않습니다.

- **`@cosmos_repository(map_to=Entity)`**: 클래스 레벨 데코레이터를 통해 모든 리포지토리 메서드에 공통 예외 처리와 엔티티 자동 매핑을 적용합니다.
- 서비스 계층은 오직 인터페이스에 의존하며, 리포지토리로부터 이미 포장된 **도메인 엔티티**를 전달받습니다 (**의존성 역전 원칙**).

### 3. Subject-Oriented Domain Models

단순한 데이터 저장소(Anemic Domain Model)를 지양하고, 엔티티 스스로가 자신의 상태를 변경하고 검증하는 **행위 중심의 모델**을 지향합니다. (예: `agent.update_version()`, `agent.is_same_version()`)
...
### 🛠️ 데이터베이스 초기화 (Initial Setup)

`docker-compose up`을 실행하면 다음 인프라가 자동으로 준비됩니다:

1.  **Cosmos DB 에뮬레이터**: `log-doctor- **handshake 서비스 활성화**: 클라이언트 에이전트의 핸드쉐이크 요청을 처리하기 위한 `/api/v1/agent/handshake` 엔드포인트를 활성화합니다.

- **Cosmos DB 초기 데이터 생성**: `python scripts/setup_db.py` 명령어를 통해 테스트용 테넌트(`test-tenant`) 정보를 데이터베이스에 미리 생성합니다.

만약 수동으로 초기화가 필요한 경우 다음 명령어를 사용하세요:
