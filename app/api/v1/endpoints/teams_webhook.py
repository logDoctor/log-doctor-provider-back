...
from app.domains.agent.dependencies import get_admin_uninstall_use_case
from app.domains.agent.usecases.admin_uninstall_use_case import AdminUninstallUseCase
...
async def receive_webhook(
    request: Request,
    uninstall_use_case: AdminUninstallUseCase = Depends(get_admin_uninstall_use_case)
):
    """
    ...
    if activity_type == "installationUpdate":
        ...
        if action == "remove":
            logger.info("Teams App Uninstall Detected via Webhook", tenant_id=tenant_id, user_id=user_id)
